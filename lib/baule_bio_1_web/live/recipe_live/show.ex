  ...
  def render(assigns) do
    ...
    <!-- Ingrédients (si on en a) -->
        <%= if @recipe.recipe_ingredients != [] do %>
          <div class="bg-white rounded-lg shadow-sm border p-6 mb-8">
            ...
            <ul class="space-y-2">
              <li
                :for={recipe_ingredient <- @recipe.recipe_ingredients}
    ...
    <!-- Navigation -->
        <div class="flex justify-between items-center">
          <.link
            navigate={recipe_index_path(@current_scope)}
            class="text-indigo-600 hover:text-indigo-800 flex items-center gap-2"
          >
            <.icon name="hero-arrow-left" class="w-4 h-4" /> Retour aux recettes
          </.link>
